...
def train():
    ...
    for episode in range(MAX_EPISODES):
        ...
        r_sk[1, :] = 0.2
        r_sk[2, :] = 0.13
        # r_sk = np.ones([4, 4]) * 0.25 * np.random.uniform()
...
if __name__ == '__main__':
    RL = DeepQNetwork(n_actions=81, n_features=8,
                      learning_rate=0.01,
                      reward_decay=0.9,
                      e_greedy=0.93,
    ...
    train()
    # sys_sat_list = np.load('sys_sat_list.npy')
    # sys_RU_list = np.load('sys_RU_list.npy')
    # plot(sys_RU_list, sys_sat_list)
