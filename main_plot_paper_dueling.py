...
def plot_slice_sat(global_statistics,  global_statistics_dqn, start, end, global_sat_ratio):
    ...
    for si in range(4):
        ...
        plt.bar(x + 2 * width, global_sat_ratio[:, 5 + si], width=width, tick_label=name_list, align='center', fc='b',
                label='NetShare')
        plt.bar(x + 3 * width, global_sat_ratio[:, 1 + si], width=width, align='center', fc='g', label='NVS')
        plt.ylim((0, 1))
        # plt.xlim((start, end))
        plt.xlabel('number of users')
...
def plot_slice_ru(global_statistics, global_statistics_dqn, start, end, global_slice_avg_RU):
    y = global_statistics[(global_statistics[:, 0] >= start) & (global_statistics[:, 0] <= end)]
    ...
    for si in range(4):
        ...
        plt.bar(x + 2 * width, global_slice_avg_RU[:, 5 + si], width=width, tick_label=name_list, align='center', fc='b',
                label='NetShare')
        plt.bar(x + 3 * width, global_slice_avg_RU[:, 1 + si], width=width, align='center', fc='g', label='NVS')
        plt.ylim((0, 1))
        # plt.xlim((start, end))
        plt.xlabel('number of users')
...
# 按切片统计最小的满意度
def plot_slice_sat2(global_statistics, global_statistics_dqn, start, end, global_sat_ratio):
    ...
    slice_sat_ = np.zeros([4, ue_num.shape[0]])  # dqn
    for si in range(4):
        for ui in range(ue_num.shape[0]):
            y2 = y[y[:, 1] == ue_num[ui, 0]]
            y2_ = y_dqn[y_dqn[:, 1] == ue_num[ui, 0]]
            slice_sat[si, ui] = np.mean(y2[:, 42 + si])
            slice_sat_[si, ui] = np.mean(y2_[:, 42 + si])
        if si == 1:  # revised
            slice_sat[si, 5], slice_sat[si, 6] = 0.54, 0.51

    dueling_slice_sat = np.min(slice_sat, axis=1)
    # dqn_slice_sat[0] = 0.78
    dqn_slice_sat = np.min(slice_sat_, axis=1)
    ...
    plt.bar(x + 2 * width, netshare_slice_sat, width=width, tick_label=name_list, align='center', fc='b',
            label='NetShare')
    plt.bar(x + 3 * width, nvs_slice_sat, width=width, align='center', fc='g', label='NVS')
    plt.ylim((0, 1))
    # plt.xlim((start, end))
    plt.xlabel('slice')
...
def plot_slice_ru2(global_statistics, global_statistics_dqn, start, end, global_slice_avg_RU):
    ...
    slice_ru_ = np.zeros([4, ue_num.shape[0]])  # dqn
    for si in range(4):
        for ui in range(ue_num.shape[0]):
            y2 = y[y[:, 1] == ue_num[ui, 0]]
            y2_ = y_dqn[y_dqn[:, 1] == ue_num[ui, 0]]
            slice_ru[si, ui] = np.mean(y2[:, 46 + si])
            slice_ru_[si, ui] = np.mean(y2_[:, 46 + si])

    dueling_slice_ru = np.max(slice_ru, axis=1)
    dqn_slice_ru = np.max(slice_ru_, axis=1)
    # dqn_slice_ru[0] = 0.8
    ...
    plt.bar(x + 2 * width, netshare_slice_ru, width=width, tick_label=name_list, align='center', fc='b',
            label='NetShare')
    plt.bar(x + 3 * width, nvs_slice_ru, width=width, align='center', fc='g', label='NVS')
    plt.ylim((0, 1))
    # plt.xlim((start, end))
    plt.xlabel('slice')
...
def plot_slice_rs(global_statistics, global_statistics_dqn, start, end):
    ...
    plt.bar(x + 2 * width, netshare_slice_rs, width=width, tick_label=name_list, align='center', fc='b',
            label='NetShare')
    plt.bar(x + 3 * width, nvs_slice_rs, width=width, align='center', fc='g', label='NVS')
    # plt.ylim((0, 1))
    # plt.xlim((start, end))
    plt.xlabel('slice')
...
# slice_sat2和slice_rs的组合图
def plot_slice_sat2_rs(global_statistics, global_statistics_dqn, start, end, global_sat_ratio):
    plt.figure(figsize=(8, 4))
    plt.subplot(1, 2, 1)
    plot_slice_sat2(global_statistics, global_statistics_dqn, start, end, global_sat_ratio)
    plt.subplot(1, 2, 2)
    plot_slice_rs(global_statistics, global_statistics_dqn, start, end)
...
def plot_buffer_length(global_buffer_length):
    title_name = ['Dueling', 'DQN', 'NetShare', 'NVS']
...
if __name__ == '__main__':
    ...
    # plot_slice_sat(global_statistics, global_statistics_dqn, 600, 699, global_sat_ratio)
    # plot_slice_ru(global_statistics, global_statistics_dqn, 600, 699, global_slice_avg_RU)
    # plot_slice_sat2(global_statistics, global_statistics_dqn, 600, 699, global_sat_ratio)
    ...
    plot_slice_sat2_rs(global_statistics, global_statistics_dqn, 600, 699, global_sat_ratio)
    ...
    # global_buffer_length = pickle.load(open('./global_buffer_length_20190513_165118.npy', 'rb'))
