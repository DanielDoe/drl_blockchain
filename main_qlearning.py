...
from DRL_Resouce_Allocation.q_learning import QLearningTable
...
def train():
    ...
    for episode in range(MAX_EPISODES):
        ...
        for j in range(MAX_EP_STEPS):
            ...
            RL.learn(get_state(j, observation[4:]), action, np.min(reward[0:4]),
                     get_state(j + 1, next_state[4:]))
...
# convert tuple(slice_load, slice_sat_ratio) to state index
# def get_state(slice_load, slice_sat_ratio, n):
#     state = 0
#     index = 7
#     for si in range(4):
#         for j in range(n):
#             if (j != n - 1) and (slice_load[si] >= j / n) and (slice_load[si] < (j + 1) / n):
#                 state += (j * n**index)
#                 break
#             elif (j == n - 1) and (slice_load[si] >= j / n) and (slice_load[si] <= (j + 1) / n):
#                 state += (j * n**index)
#                 break
#         index -= 1
#
#     for si in range(4):
#         for j in range(n):
#             if (j != n - 1) and (slice_sat_ratio[si] >= j / n) and (slice_sat_ratio[si] < (j + 1) / n):
#                 state += (j * n**index)
#                 break
#             elif (j == n - 1) and (slice_sat_ratio[si] >= j / n) and (slice_sat_ratio[si] <= (j + 1) / n):
#                 state += (j * n**index)
#                 break
#         index -= 1
#     return state
...
def get_state(slice_load, slice_sat_ratio):
    state = '{:03b}'.format(slice_load)
    for si in range(4):
        if slice_sat_ratio[si] < 0.5:
            state += '0'
        else:
            state += '1'
    return int(state, 2)  # 二进制转化为十进制


if __name__ == '__main__':
    RL = QLearningTable(n_features=2**7,
